...
export async function requestWithdrawal(userId, amountKobo) {
  const db = requireDatabase();
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    ...
    const request = await client.query("INSERT INTO withdrawal_requests (user_id,amount_kobo,fee_kobo,net_kobo,status) VALUES ($1,$2,0,$2,$$pending$$) RETURNING id,amount_kobo,fee_kobo,net_kobo,status,created_at", [userId, amountKobo]);
    ...
    await client.query("INSERT INTO wallet_ledger (user_id,type,amount_kobo,reference,status,metadata) VALUES ($1,$$withdrawal_hold$$,$2,$3,$$posted$$,$4::jsonb)", [userId, -amountKobo, makeReference("wdhold"), JSON.stringify({withdrawalId:request.rows[0].id})]);
    await client.query("COMMIT");
    return { ok:true, withdrawal:request.rows[0], message:"Withdrawal request created and held for payout processing." };
